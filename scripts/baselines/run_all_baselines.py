...
def _discover_unified_groups(repo: Path) -> list[str]:
    ...
    if not unified_dir.exists():
        raise FileNotFoundError(
            ...
            "Run the unified export first or pass --group-ids explicitly."
        )
...
def main() -> int:
    p = argparse.ArgumentParser(description="Run all baseline models across datasets.")
    ...
    p.add_argument("--group-ids", nargs="+", default=None)
    ...
    if args.data_mode == "unified":
        targets = args.group_ids or _discover_unified_groups(repo)
    ...
    jobs = [(m, target) for m in args.models for target in targets]
    total = len(jobs)
    for i, (model, target) in enumerate(jobs, start=1):
        log_dir = repo / DEFAULT_RESULTS_DIR / model / args.data_mode / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)
        target_label = f"D{target}" if args.data_mode == "processed" else str(target)
        log_path = log_dir / f"{model}_{_safe_label(str(target_label))}.log"
        print("\n" + "═" * 72)
        print(f"[{i}/{total}] Running {model} on {target_label}")
        print(f"Log: {log_path}")
        print("═" * 72 + "\n")
        cmd = [
            sys.executable,
            f"scripts/baselines/run_{model}.py",
        ...
        with open(log_path, "w", encoding="utf-8") as logf:
            ...
            rc = proc.wait()
        if rc != 0:
            print(f"  {model} {target_label}: FAILED (exit {rc})")
            if args.stop_on_failure:
                return rc
        else:
            print(f"  {target_label}: OK")
    return 0
